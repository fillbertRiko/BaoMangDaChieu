...
if (isset($_POST['thembaiviet']) && !empty($title) && !empty($content) && !empty($summary) && !empty($source) && !empty($published_at) && !empty($category)) {
    ...
    $sql_add = "INSERT INTO articlesmost (category, title, publication_date, summary, content, source, image_url) 
                VALUES ('$category', '$title', '$published_at', '$summary', '$content', '$source', '$image_url')";

    if (mysqli_query($mysqli, $sql_add)) {
        header('Location: ../../index.php?action=quanlyarticlesmost&query=them');
        exit();
    } else {
...
elseif (isset($_POST['suabaiviet'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if (move_uploaded_file($image["tmp_name"], $target_file)) {
            $image_url = "/uploads/" . basename($image["name"]); // Địa chỉ của hình ảnh
            $sql_update = "UPDATE articlesmost SET category='$category', title='$title', publication_date='$published_at', summary='$summary', content='$content', source='$source', image_url='$image_url' WHERE article_id='$article_id'";
        } else {
            echo "Có lỗi xảy ra khi tải lên hình ảnh.";
            exit();
        }
        ...
        $sql_update = "UPDATE articlesmost SET category='$category', title='$title', publication_date='$published_at', summary='$summary', content='$content', source='$source' WHERE article_id='$article_id'";
    }

    if (mysqli_query($mysqli, $sql_update)) {
        header('Location: ../../index.php?action=quanlyarticlesmost&query=sua');
        exit();
    } else {
        echo "Lỗi: " . mysqli_error($mysqli);
    }
...
elseif (isset($_GET['query']) && $_GET['query'] == 'delete') {
    ...
    $sql_delete = "DELETE FROM articlesmost WHERE article_id = '$article_id'";

    if (mysqli_query($mysqli, $sql_delete)) {
        header('Location: ../../index.php?action=quanlyarticlesmost&query=xoa');
        exit();
    } else {
        echo "Lỗi: " . mysqli_error($mysqli);
    }
